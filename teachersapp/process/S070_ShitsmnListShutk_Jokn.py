...
def main(keyWord, list_hashTag, date_From, date_To, selfQ_userID, selfRQ_userID):
    ...
    try:
                        ...
                        from (   \
                                select MAX(t100.SHITSMN_ID) as SHITSMN_ID \
                                    ...
                                    left outer join t120_kaitrequest t120 \
                                        on t100.SHITSMN_ID = t120.SHITSMN_ID \
                                        AND t120.DELFLG = '0' \
                                    where t100.DELFLG = '0' "
        if not C050_StringUtil.isNull(keyWord):
            sql = sql + \
                "               AND ((t100.SHITSMN_TITLE like %s) or (t100.SHITSMN_NAIYO like %s))"
            keyWord = "%" + keyWord + "%"
        ...
        if (not C050_StringUtil.isNull(date_From)) and (not C050_StringUtil.isNull(date_To)):
            #sql = sql + "               AND t100.KAISHNCHJ01 >= %s "
            sql = sql + "                				AND ( \
						(t100.KAISHNCHJ01 >= %s AND t100.KAISHNCHJ01 <= %s) OR (t100.KAISHNCHJ02 >= %s AND t100.KAISHNCHJ02 <= %s) OR (t100.KAISHNCHJ03 >= %s AND t100.KAISHNCHJ03 <= %s) \
					) "
        ...
        json_service = {"json_CommonInfo": json_CommonInfo,
                        "tuple_T100_shitsmnList_jokn": rows}
        return json_service
    # ==例外処理==========================================================================================
