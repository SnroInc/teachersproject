...
def main(shitsmnID, int_seq, kaitUserID, kaitUserComment, rqYukJikn, rqYukKign, matchingFlg):
    # --戻り値用の変数宣言------------------------------------------------------------------------------
    errflg = "0"
    list_msgInfo = []
    json_shitsmnInfo = {}
    ...
    try:
        ...
        int_rqSeq = C020_DBUtil.executeSQL(
            json_DBConnectInfo, sql_rqSeq, args_rqSeq)[0]["NEWRQSEQ"]
        # --採番処理呼び出し----------------------------------------------------------------------------
        # --DB連携基本コード----------------------------------------------------------------------------
        # DB接続開始、コネクションとカーソルを取得
        #json_DBConnectInfo = C020_DBUtil.connectDB()
        # --(2)クエリとパラメータを定義
        sql = "INSERT INTO T120_KAITREQUEST VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,current_timestamp(6),%s,%s,current_timestamp(6),%s);"
        args = (shitsmnID, int_seq, int_rqSeq, kaitUserID, kaitUserComment,
                rqYukJikn, rqYukKign, matchingFlg, SERVICE_ID, kaitUserID, SERVICE_ID, kaitUserID, "0")
        # クエリを実行し、結果を取得
        # T100
        ...
        json_service = {"json_CommonInfo": json_CommonInfo,
                        "shitsmnID": shitsmnID, "int_seq": int_seq, "int_rqSeq": int_rqSeq}
        return json_service
    # ==例外処理==========================================================================================
