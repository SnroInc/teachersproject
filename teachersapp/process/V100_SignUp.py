        ...
        if request.method == 'POST':
            ...
            if C050_StringUtil.isAllSpace(userName):
                ...
                C030_MessageUtil.setMessage(
                    request, "E0001", (itemName_userName,))
            # メールアドレス
            if C050_StringUtil.isAllSpace(mailAddress):
                errflg = "1"
                C030_MessageUtil.setMessage(
                    request, "E0001", (itemName_mailAddress,))
            # ログインID
            if C050_StringUtil.isAllSpace(loginID):
                errflg = "1"
                C030_MessageUtil.setMessage(
                    request, "E0001", (itemName_loginID,))
            # ログインパスワード
            if C050_StringUtil.isAllSpace(loginPass):
                errflg = "1"
                C030_MessageUtil.setMessage(
                    request, "E0001", (itemName_loginPass,))
            # ログインパスワード（確認用）
            if C050_StringUtil.isAllSpace(loginPass_conf):
                errflg = "1"
                C030_MessageUtil.setMessage(
                    request, "E0001", (itemName_loginPass_conf,))
            # 単項目チェック-------------------------------------------------------------------------------------------------
            ...
            if not C050_StringUtil.isSameCharacter(loginPass, loginPass_conf):
                errflg = "1"
                C030_MessageUtil.setMessage(
                    request, "E0005", (itemName_loginPass, itemName_loginPass_conf,))
            # パスワードとパスワード（確認用）の一致チェック-----------------------------------------------------------------------

            # DBに同じメールアドレスが登録されていないかのチェック--------------------------------------------------------------------
            json_S185 = S910_TableItemCounter.main(
                "M050_USER", "MAIL_ADDRESS", mailAddress, "0")
            ...
            if counter_loginID > 0:
                ...
                # 「このメールアドレスは使えません」
                C030_MessageUtil.setMessage(
                    request, "E0004", (itemName_mailAddress,))
            # DBに同じメールアドレスが登録されていないかのチェック--------------------------------------------------------------------
            # DBに同じログインIDが登録されていないかのチェック--------------------------------------------------------------------
            json_S185 = S910_TableItemCounter.main(
                "M050_USER", "LOGINID", loginID, "0")
            #errflg_S185 = json_S185["json_CommonInfo"]["errflg"]
            list_msgInfo_S185 = json_S185["json_CommonInfo"]["list_msgInfo"]
            # メッセージ格納
            # C030_MessageUtil.setMessageList(request,list_msgInfo_S185)
            ...
            if counter_loginID > 0:
                ...
                # 「このユーザIDは使えません」
                C030_MessageUtil.setMessage(
                    request, "E0004", (itemName_loginID,))
            # DBに同じログインIDが登録されていないかのチェック--------------------------------------------------------------------
            ...
            # --S150-------------------------------------------------------------------------
            json_S150 = S150_UserInfoTork.main(
                userName, mailAddress, loginID, loginPass, hyoka, userComment, loginKbn)
            # 個々の値を取得
