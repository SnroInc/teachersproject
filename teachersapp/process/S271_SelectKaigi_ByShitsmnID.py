...
def main(shitsmnID, loginUserID):
    # --戻り値用の変数宣言------------------------------------------------------------------------------
    errflg = "0"
    list_msgInfo = []
    json_kaigiInfo = {}
    ...
    try:
        ...
        # クエリを定義
        sql = "select KAIGI_ID,KAIGI_PATH,SHITSMN_USERID,KAIT_USERID,SHITSMN_ID,SEQ,STR_DATETIME,END_DATETIME,KAIGITIME from T130_KAIGI where SHITSMN_ID = %s and (SHITSMN_USERID = %s or KAIT_USERID = %s) and DELFLG = %s ;"
        # パラメータを定義
        args = (shitsmnID, loginUserID, loginUserID, "0",)
        # クエリを実行し、結果を取得
