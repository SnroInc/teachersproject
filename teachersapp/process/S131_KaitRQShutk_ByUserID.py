...
def main(shitsmnID, int_seq, kaitUserID):
    # --戻り値用の変数宣言------------------------------------------------------------------------------
    errflg = "0"
    list_msgInfo = []
    json_kaitRQInfo = {}
    ...
    try:
        ...
        # クエリを定義
        sql = "select SHITSMN_ID, SEQ, RQSEQ, KAIT_USERID, KAIT_USERCOMMENT, RQYUKJKN, RQYUKKGN, MATCHINGFLG, CRTUSR, CRTDATE, UPDUSR, UPDDATE from T120_KAITREQUEST where SHITSMN_ID = %s and SEQ = %s and KAIT_USERID = %s and DELFLG = %s order by RQSEQ DESC;"
        # パラメータを定義
        ...
        json_service = {"json_CommonInfo": json_CommonInfo,
                        "json_kaitRQInfo": json_kaitRQInfo}
        return json_service
    # ==例外処理==========================================================================================
